...
- [Print = one leaf per sheet via JS scaling](#) — printing all content flat gave ~10 sheets because each leaf (steps, worksheet, colophon) is taller than one Letter page. **Fix:** `beforeprint` adds `body.printing` (+lowink), wraps each `.page`'s children in `.printwrap`, measures `scrollHeight` vs the page's available content height, and applies `transform:scale()` to shrink over-tall leaves to fit; `afterprint` unwraps. **Why a body class (not just @media print):** JS measurements run in screen media, so the print layout MUST be expressed as a `body.printing` class for getBoundingClientRect/scrollHeight to match what prints. The `@media print` block mirrors the same fixed sheet sizing (`.page` 8.5in×10.9in, padding 13mm, `@page margin:0`, break-after:page) as a no-JS fallback. Per-page folio `.pfoot` is hidden in print to avoid transform-containing-block misplacement. Result: cover + 5 content = 6 sheets.
