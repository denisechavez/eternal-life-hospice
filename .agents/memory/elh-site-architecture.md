...
- **comfort-therapies heart hero — decided (do not redo):** keep the full-bleed `.hero--photo` fed by the blurred-fill heart photo (`comfort-therapies.jpg` / `-hero.jpg`, 2000x900, byte-identical, also the resources.html card thumbnail). The user explicitly does NOT want this photo swapped for a tight heart crop, and does NOT want a separate round heart-above-copy layout (that was tried and reverted). To make the heart bigger, REBUILD the blurred-fill: reuse the original blurred background and composite a LARGER centered sharp heart crop on top — the clean 1280x1280 source heart photo is recoverable from git history. Don't replace the whole image.
