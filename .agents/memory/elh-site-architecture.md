...
- **The brand horizontal files are SINGLE-color (all cream / all plum). The user wants "LIFE HOSPICE" subtitle in GOLD while the mark + "Eternal" keep the base color — so the header logos are recolored, not the raw brand files.** cream variant: base `#F5F0EB`, subtitle `#C9B07E` (brand gold, pops on dark). plum variant: base `#4A1B46` (rich plum, matches `--deep`), subtitle `#9A7E4A` (deep gold, readable on cream).
