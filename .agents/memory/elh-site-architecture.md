...
- **comfort-therapies heart hero — current state (user is VERY particular here; went through ~3 pivots: blurred-fill → tight crop → bigger heart in blurred-fill → finally the full uncropped photo).** Landing decision: show the ENTIRE heart photo with NO crops. Hero is a plain plum `.hero` (NOT `.hero--photo`, whose background-cover crops) containing `<img class="hero-photo-full">` (full image, contained, `max-width:760px`, `height:auto`) above the kicker/h1/copy. Source = the full ~4096x3472 photo resized to 1600x1356 (`comfort-therapies.jpg` / `-hero.jpg`, byte-identical; resources.html grid card still cover-crops the same file as a small thumbnail, which is fine). Don't reintroduce cover-cropping or the blurred-fill composite on this hero.
