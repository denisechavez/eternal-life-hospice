...
- **Header = the HORIZONTAL lockup** (`-trim -resize 900x`) → `assets/img/elh-logo-h-cream.png` (on dark header) + `elh-logo-h-plum.png` (on `#hdr.scrolled`), at `.hdr-logo img.s{height:46px}`. The `.hdr-wordmark` CSS text is hidden — the image is the whole logo (mark + logotype font). Don't hand-build a horizontal logo by compositing the stacked one; use the official horizontal file.
