...
**Hospital domains verify with huge "unknown" rates on NeverBounce** (greylisting/timeouts): a 3,846-email batch returned only ~945 valid (~25%), 867 catch-all, 1,988 unknown. So expect a LOW valid yield from health-system scrapes; "verified only" = `result=valid`, exclude catch-all + unknown + invalid. NeverBounce job needs ≥ 1 paid credit per email (bulk `jobs/create` with `input_location:supplied`, `auto_parse+auto_start`; `jobs/download` with `valids:1` for a valid-only CSV).
