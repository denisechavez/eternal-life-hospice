...
(function () {
  ...
  function injectStyles() {
    var css = [
      ...
      ".elhc-panel{position:fixed;right:24px;bottom:0;z-index:2147483000;width:410px;max-width:100vw;height:min(620px,92vh);background:var(--cream);border-radius:22px 22px 0 0;box-shadow:0 -20px 70px rgba(60,28,59,.28);display:none;flex-direction:column;overflow:hidden;border:1px solid var(--cdark);border-bottom:none}",
      "@media(max-width:480px){.elhc-panel{right:0;left:0;width:100%;height:88vh;height:88dvh}}",
      ".elhc-panel.open{display:flex;animation:elhcSlideUp .44s cubic-bezier(.22,1,.36,1)}",
      ".elhc-panel.closing{display:flex;animation:elhcSlideDown .3s cubic-bezier(.4,0,1,1) forwards}",
      "@keyframes elhcSlideUp{from{transform:translateY(100%)}to{transform:none}}",
      "@keyframes elhcSlideDown{from{transform:none}to{transform:translateY(100%)}}",
